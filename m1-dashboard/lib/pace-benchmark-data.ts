/**
 * 과거 예약 데이터 기반 판매 객실수 벤치마크
 * raw_booking_history 테이블에서 자동 생성됨
 *
 * 키: 'branch|roomtype|month|dow_type'
 * 값: { lead_time_days(string): expected_sold_rooms (정수) }
 *   예: { "14": 15 } → D-14 시점에 역사적으로 15실 판매
 */
...
/**
 * 해당 리드타임의 과거 판매 객실수 조회
 * @returns 과거 중앙값 판매 객실수 또는 null (데이터 없음)
 */
export function getExpectedSold(
  branchName: string,
  roomType: string,
  month: number,
  ...
  for (let i = 0; i < buckets.length - 1; i++) {
    if (leadTimeDays >= buckets[i] && leadTimeDays <= buckets[i + 1]) {
      ...
      return Math.round(curve[String(buckets[i])] + ratio * (curve[String(buckets[i + 1])] - curve[String(buckets[i])]))
    }
  }
